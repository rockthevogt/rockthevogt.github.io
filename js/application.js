...
/**
 * Should we enable the middle finger modal?
 *
 * @return boolean
 */

function overlayDisabled() {
	// Is the querystring param provided? If so, obey it.
	var url = new URL(window.location.href);
	if (url.searchParams.has('overlay')) {
		var paramValue = url.searchParams.get('overlay');
	...
	// Is a cookie defined?
	var cookieValue = Cookies.get('overlayDisabled');
	if (typeof(cookieValue) === 'undefined') {
		// No cookie, no qs param. Don't disable the overlay
		return false;
	}
...
$(function() {
	if (overlayDisabled()) {
		Cookies.set('overlayDisabled', 'true');
		$('#fuck-you-overlay').remove();
	} else {
		// The true JOHNVOGT mobile experience
		Cookies.remove('overlayDisabled');
	}
});
